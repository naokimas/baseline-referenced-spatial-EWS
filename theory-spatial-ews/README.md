...
Figure 1 uses five networks: one synthetic and four empirical. The empirical networks are not ours.
Therefore, we do not upload them here, so the four corresponding
...
**Where to get the four empirical networks.** See `data/networks/README.md` of the parent repository.
...
`fig_ews_examples.pdf` into it. Those two files are the figure in the manuscript.
...
| section S2, numerical method | `generate_equilibria.py`; ranges and $\Delta t$ in `data/simulation_parameters.csv` |
| section S2, home range | `common/home_range.py`; the resulting lengths are the `home_len` column of `data/equilibria/manifest.csv` |
| section S2, networks | `data/networks/`; see the table below |
